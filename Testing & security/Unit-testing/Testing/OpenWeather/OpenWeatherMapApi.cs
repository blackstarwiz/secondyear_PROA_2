...
namespace TempAntwerpen
{
    public class OpenWeatherMapApi : IOpenWeatherMapApi
    {
        ...
        {
            var httpResponse = _client.GetStringAsync(url).GetAwaiter().GetResult();//krijg een json response
            ...
            var weather = JsonConvert.DeserializeObject<OpenWeather_AntwerpTemp>(httpResponse);
